  ...
  try {
    ...
    switch (action) {
      ...
      case "sync_chats": {
        ...
        const chatsRes = await fetch(`${baseUrl}/chat/findChats/${name}`, {
          method: "POST",
          headers,
          body: JSON.stringify({}),
        });
        const chats = await chatsRes.json();
        if (!chatsRes.ok) return json({ error: "Failed to fetch chats", details: chats }, chatsRes.status);

        // Fetch contacts from Evolution API
        const contactsRes = await fetch(`${baseUrl}/chat/findContacts/${name}`, {
          method: "POST",
          headers,
          body: JSON.stringify({}),
        });
        ...
        if (Array.isArray(contacts)) {
          for (const c of contacts) {
            ...
            const cName = c.pushName || c.name || c.verifiedName || "";
            if (jid && cName) contactMap[jid] = cName;
          }
        }
        ...
        for (const chat of chatList) {
          ...
          const contactName = contactMap[remoteJid] || chat.name || chat.pushName || phone;
          ...
          await supabase.from("conversas").insert({
            empresa_id: empresaId,
            contato_nome: cliente?.nome || contactName,
        ...
        return json({ success: true, synced, total: chatList.length });
      }
