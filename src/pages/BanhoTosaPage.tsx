...
import { Scissors, CalendarDays, List, PawPrint, Phone, MessageCircle, Pencil, LogIn, Trash2, XCircle } from "lucide-react";
...
export default function BanhoTosaPage() {
  ...
  // Filter agendamentos matching banho/tosa keywords for the list view
  const banhoTosaAgendamentos = useMemo(() => {
  ...
  const serviceTypes = useMemo(() => {
    const types = new Set(banhoTosaAgendamentos.map(a => a.tipo_servico));
    return Array.from(types).sort();
  }, [banhoTosaAgendamentos]);
  ...
  }, [banhoTosaAgendamentos, filterDate, filterService]);
      ...
      <Tabs value={view} onValueChange={v => setView(v as any)}>
        ...
        <TabsContent value="calendar" className="mt-4">
          <AgendaCalendar
            ...
            onEditAgendamento={(a) => setEditingAgendamento(a as any)}
            serviceKeywords={BANHO_TOSA_KEYWORDS}
          />
        </TabsContent>
        ...
        <TabsContent value="list" className="mt-4 space-y-4">
          <div className="flex flex-wrap items-center gap-3">
            <div className="space-y-1">
              ...
              <Input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} className="w-[160px] h-9" />
            </div>
            <div className="space-y-1">
...
function BanhoTosaRow({ item, onCheckin, onEdit, onDelete, onFalta }: {
  item: Agendamento;
  onCheckin: (item: Agendamento) => void;
  onEdit: (a: Agendamento) => void;
  onDelete: (id: string) => void;
  onFalta: (item: Agendamento) => void;
}) {
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-3 hover:bg-muted/30 transition-colors">
      ...
      <div className="flex items-center gap-1 shrink-0">
        {item.status !== "na_empresa" && item.status !== "concluido" && item.status !== "falta" && (
          <>
